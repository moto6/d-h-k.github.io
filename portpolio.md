...
- OOP Framework 도입 
  - 기존의 소스코드가 SuperLoop 속에 모든 Logic이 들어있어 기능추가할 때 마다 영향이 가는 side-effect가 심했습니다. 버그가 계속해서 발생할 뿐 아니라 재현하기도 힘들어 원인을 찾아 문제를 해결하는데 어려움이 많았습니다. 
  - 유지보수가 어려워 한계에 다다른 상황였습니다. 한가지 기능 변경을 위해서는 소스코드 이곳저곳 여러군데를 돌아다니며 반복적인 Copy&Paste 작업을 해야할 뿐만 아니라, 모든 장비별로 각각의 프로젝트와 빌드파일로 관리해야하며, 하나의 쓰레드에 모든 로직이 들어있어 기능추가도 어려웠습니다. 
  - 위 문제들을 해결하기 위해 OOP FrameWork를 도입했고, 그 결과 유지보수가 쉬워졌을 뿐 아니라 서로 다른 하드웨어와 시뮬레이터 장비라도 한가지 빌드파일에서 동작하는 통합미들웨어를 개발해 성공적으로 적용했습니다.
- 장비 제어 알고리즘 개발 
  - 컨텐츠에서 나오는 움직임만으로는 장비의 물리적인 가동범위를 온전히 활용하지 못하는 문제점이 있었습니다. 
  - 사실적인 움직임보다는 탑승객들의 재미를 극대화하기 위해 모터의 PID 게인을 튜닝하고 모터 이동명령에 비선형 필터를 추가해서 불쾌한 잔 진동을 줄이고 가속& 감속시 확실하고 큰 모션 피드백을 구현했습니다.
- 하드웨어 의존성 추상화 라이브러리 개발
   - 근무 중 사용하던 PCI-E 통신으로 연결되는 NativeAPI 모션 컨트롤러가 단종되었고, 이를 대체하기 위해서 Ethernet 케이블로 연결되어 DLL을 호출하는 방식의 컨트롤러를 도입해 시뮬레이터에 통합하는 작업을 진행했습니다. 
   - 이 통합작업을 진행하며 기존방식과 다르게 동작하는 함수들을 추상화해서 기존에 동작하는 NativeAPI 에 1:1 대체할수 있도록 라이브러리를 작성했고, 제어하는 모터 변경시 Delay가 있었는데 모터 제조사 엔지니어와 협업을 통해 Delay 없이 제어할 수 있도록 DLL 을 수정해 받는 작업을 진행했습니다. 그 결과 OOP 미들웨어에 성공적으로 통합할 수 있었습니다.
